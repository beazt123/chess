                ...
                if (forwardMove == 8 && !destinationTile.isTileOccupied()){
                    ...
                    if ((destinationTile.isTileOccupied() && pieceAtTile.getPieceAlliance() != this.pieceAlliance)
                            && ((this.pieceAlliance.isWhite()
                                    && !BoardUtils.EIGHTH_COLUMN[piecePosition])
                                || (this.pieceAlliance.isBlack()
                                    && !BoardUtils.FIRST_COLUMN[piecePosition]))){
                        if(this.pieceAlliance.isPawnPromotionSquare(destinationCoordinate)) {
                            legalMoves.add(new Move.PawnPromotion(new Move.PawnAttackMove(board, this, destinationCoordinate, pieceAtTile)));
                        } else
                            legalMoves.add(
                        ...
                        if (this.pieceAlliance != pieceOnCandidate.getPieceAlliance()) {
                            legalMoves.add(
                                    new Move.PawnEnPassantAttackMove(board,
                                            this,
                                            destinationCoordinate,
                                            pieceOnCandidate)
                            );
